...
{
    /* TODO: This version breaks one test because an attribute 'a' is missng from some array.
       Haven't debugged it further than that yet. */
#if 0    
    // If m2 is an array, use the faster core routines.
    if (PyArray_Check(m1) && PyArray_Check(m2)) {
        NpyArray *ret = NULL;
