        ...
        int byteorder, int aligned, int writeable)
{
    ...
    if (result) {
        if (!NA_NumArrayCheck((PyObject *) result)) {
            ...
            if (buffer) {
                memcpy(PyArray_BYTES(result), buffer, NA_NBYTES(result));
            } else {
                memset(PyArray_BYTES(result), 0, NA_NBYTES(result));
            }
        }
    }
    return result;
        ...
        int byteorder, int aligned, int writeable)
{
    ...
    PyArrayObject *result = NA_NewAll(ndim, shape, type, buffer,
            ...
            byteorder, aligned, writeable);
    for(i=0; i<ndim; i++)
        PyArray_STRIDE(result, i) = strides[i];
    return result;
}
        ...
        CFUNC_STRIDED_FUNC f)
{
    int i, j, status=0;
    dim -= 1;
    for(i=0; i<PyArray_DIM(numarray[0],dim); i++) {
        for (j=0; j<nnumarray; j++)
            data[j] += PyArray_STRIDE(numarray[j], dim)*i;
        if (dim == 0)
...
callStridingCFunc(PyObject *self, PyObject *args) {
    ...
    for(i=0; i<nnumarray; i++) {
        ...
        data[i] = PyArray_BYTES(numarray[i]);
        Py_DECREF(otemp);
...
long NA_get_offset(PyArrayObject *a, int N, ...)
{
    ...
    if (N > 0) { /* compute offset of "outer" indices. */
        for(i=0; i<N; i++)
            offset += va_arg(ap, long) * PyArray_STRIDE(a, i);
    } else {   /* compute offset of "inner" indices. */
        N = -N;
        for(i=0; i<N; i++)
            offset += va_arg(ap, long) * PyArray_STRIDE(a, PyArray_NDIM(a)-N+i);
    }
    va_end(ap);
    return offset;
}
...
{
    long offset = i * PyArray_STRIDE(a, 0);
...
NA_ShapeLessThan(PyArrayObject *a, PyArrayObject *b)
{
    ...
    for(i=0; i<mindim; i++)
        if (PyArray_DIM(a, i+aoff) >=  PyArray_DIM(b, i+boff))
            return 0;
    return 1;
}
...
getShape(PyObject *a, maybelong *shape, int dims)
{
    ...
    if (!PySequence_Check(a) ||
        (NA_NDArrayCheck(a) && (PyArray_NDIM(a) == 0)))
        return dims;
    ...
    for(i=0; i<slen; i++) {
        ...
        if (!o) {
            ...
            return -1;
                    ...
                    (NA_NumArrayCheck(o) && PyArray_NDIM(o) == 0)) &&
                         ...
                         int entries)
{
    ...
    if (entries >= 100) {
        ...
        if (PyArray_TYPE(a) == tInt64) {
            v = (Int64) PyLong_AsLongLong( value );
        } else if (PyArray_TYPE(a) == tUInt64) {
            v = (UInt64) PyLong_AsUnsignedLongLong( value );
        } else if (PyArray_TYPE(a) == tUInt32) {
        ...
        int rval;
        PyObject *type = NA_typeNoToTypeObject(PyArray_TYPE(a));
        if (!type) return -1;
...
NA_swapAxes(PyArrayObject *array, int x, int y)
{
    ...
    temp = PyArray_DIM(array, x);
    PyArray_DIM(array, x) = PyArray_DIM(array, y);
    PyArray_DIM(array, y) = temp;

    temp = PyArray_STRIDE(array, x);
    PyArray_STRIDE(array, x) = PyArray_STRIDE(array, y);
    PyArray_STRIDE(array, y) = temp;
