...
fortran_doc (FortranDataDef def) {
    ...
    if (def.rank==-1) {
        ...
        if (sprintf(p+strlen(p),"'%c'-",d->descr->type)==0) {
	  Py_DECREF(d);
	  goto fail;
	}
        Py_DECREF(d);
...
static int swap_arrays(NpyArray* arr1, NpyArray* arr2) {
    SWAPTYPE(arr1->data, arr2->data, char*);
    SWAPTYPE(NpyArray_NDIM(arr1), NpyArray_NDIM(arr2),int);
    SWAPTYPE(NpyArray_DIMS(arr1), NpyArray_DIMS(arr2),npy_intp*);
    SWAPTYPE(NpyArray_STRIDES(arr1), NpyArray_STRIDES(arr2),npy_intp*);
    SWAPTYPE(NpyArray_BASE(arr1), NpyArray_BASE(arr2), PyObject*);
    SWAPTYPE(NpyArray_DESCR(arr1), NpyArray_DESCR(arr2), NpyArray_Descr*);
    SWAPTYPE(NpyArray_FLAGS(arr1), NpyArray_FLAGS(arr2),int);
    /* SWAPTYPE(arr1->weakreflist,arr2->weakreflist,PyObject*); */
    return 0;
}

#define ARRAY_ISCOMPATIBLE(arr,type_num)                                \
    (  (PyArray_ISINTEGER(arr) && NpyTypeNum_ISINTEGER(type_num))       \
       ||(PyArray_ISFLOAT(arr) && NpyTypeNum_ISFLOAT(type_num))         \
       ||(PyArray_ISCOMPLEX(arr) && NpyTypeNum_ISCOMPLEX(type_num))     \
       ||(PyArray_ISBOOL(arr) && NpyTypeNum_ISBOOL(type_num))           \
       )
	...
	*/
...

        {
            ...
            if (intent & F2PY_INTENT_INPLACE) {
                if (swap_arrays(PyArray_ARRAY(arr), PyArray_ARRAY(retarr)))
                    return NULL; /* XXX: set exception */
                Py_XDECREF(retarr);
                if (intent & F2PY_INTENT_OUT)
                    Py_INCREF(arr);
    ...
    */
    ...
    if (rank > PyArray_NDIM(arr)) { /* [1,2] -> [[1],[2]]; 1 -> [[1]]  */
        ...
        for (i=0,effrank=0; i < PyArray_NDIM(arr); ++i)
            if (PyArray_DIMS(arr)[i]>1) ++effrank;
        if (dims[rank-1]>=0)
            if (effrank>rank) {
        ...
        for (i=0,j=0;i<rank;++i) {
            while (j < PyArray_NDIM(arr) && PyArray_DIMS(arr)[j]<2) ++j;
            if (j >= PyArray_NDIM(arr)) d = 1;
            else d = PyArray_DIMS(arr)[j++];
            if (dims[i]>=0) {
                if (d>1 && d!=dims[i]) {
                    fprintf(stderr,"%d-th dimension must be fixed to %" NPY_INTP_FMT
        ...
        if (size != arr_size) {
            ...
            for (i=0; i< PyArray_NDIM(arr); ++i) fprintf(stderr," %" NPY_INTP_FMT, PyArray_DIMS(arr)[i]);
            fprintf(stderr," ]\n");
            return 1;
        }
    }
